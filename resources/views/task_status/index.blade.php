            ...
            <tbody>
                @foreach($taskStatuses as $status)
                    <tr>
                        ...
                        @if (\Auth::check())
                        <td><a href="{{route('task_statuses.destroy', $status->id)}}" data-confirm="Вы уверены?" data-method="delete" rel="nofollow">@lang('action.delete')
                        <a href="{{route('task_statuses.edit', $status->id)}}">@lang('action.edit')</td>
                        </td>   
                        @endif
                    </tr>
                @endforeach
